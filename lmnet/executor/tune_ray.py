...
def setup_dataset(config, subset, rank):
    """helper function from nn/train.py to setup the data iterator"""
...
class TrainTunable(Trainable):
    ...
    def _setup(self, config):
        ...
        if model_class.__module__.startswith("nn.networks.object_detection"):
            model = model_class(
        ...
        elif model_class.__module__.startswith("nn.networks.segmentation"):
            model = model_class(
                classes=self.train_dataset.classes,
    ...
    def _train(self):
        ...
        if self.lm_config.NETWORK_CLASS.__module__.startswith("nn.networks.segmentation"):
