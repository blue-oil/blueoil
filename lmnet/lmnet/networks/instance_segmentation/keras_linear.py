...
def linear_mid_tread_half_quantizer(
    ...
    r"""Linear mid tread half quantizer.
        ...
        Args:
            ...
            dtype (K.tf.DType): Define the data type of args of forward and backward.
    ...
    def _backward(op, grad_quantized):
        ...
        Args:
            op (K.tf.Operation): The forward operation.
            grad_quantized (K.tf.Tensor): The gradient w.r.t quantized input, weights normally.

        Returns:
            K.tf.Variable: The gradient w.r.t. normal (non-quantized) input.

        """
        if backward:
            return backward(op, grad_quantized)
        x = op.inputs[0]
        true = K.tf.ones(K.tf.shape(x))
        false = K.tf.zeros(K.tf.shape(x))
        dx = K.tf.where((x < max_value) & (x > min_value), true, false)
    ...
    def _forward(x):
        ...
        Args:
            x (K.tf.Variable): The input to be quantized, weights normally.
        ...
        def _func(x, bit, max_value):
            ...
            x = K.tf.clip_by_value(x, min_value, max_value, name="clip")
            shifted = (x - min_value) / value_range
            quantized = K.tf.floor(shifted * n + 0.5) / n
