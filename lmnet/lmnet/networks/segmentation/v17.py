...
class LmSegnetV1(Base):
    ...
    def spatial(self, x):
        with tf.variable_scope("spatial"):
            ...
            x = self.lmnet_block('conv_2', x, 48, 3)
            x = self._space_to_depth(name='s2d_3', inputs=x)
            x = self.lmnet_block('conv_3', x, 64, 3, activation=tf.nn.relu)
    ...
    def concat_block(self, name, inputs, channel, kernel):
    ...
    def context(self, x):
        with tf.variable_scope("context"):
            with tf.variable_scope("block_1"):
                ...
                x = self.concat_block('concat_1', x, 96, 3)
                x = self.concat_block('concat_2', x, 128, 3)
                x = self.concat_block('concat_3', x, 160, 3)
                x = self.concat_block('concat_4', x, 192, 3)
                x = self.concat_block('concat_5', x, 224, 3)
                x = self.concat_block('concat_6', x, 256, 3)
                x = self.lmnet_block('conv_2', x, 64, 1)

            with tf.variable_scope("block_2"):
                x = self._space_to_depth(name='s2d_2', inputs=x)
                x = self.lmnet_block('conv_1', x, 128, 1)
                x = self.concat_block('concat_1', x, 160, 3)
                x = self.concat_block('concat_2', x, 192, 3)
                x = self.concat_block('concat_3', x, 224, 3)
                x = self.concat_block('concat_4', x, 256, 3)
                x = self.concat_block('concat_5', x, 288, 3)
                x = self.concat_block('concat_6', x, 320, 3)
            ...
            with tf.variable_scope("block_3"):
                ...
                x = self.concat_block('concat_1', x, 320, 3)
                x = self.concat_block('concat_2', x, 384, 3)
                x = self.concat_block('concat_3', x, 448, 3)
                x = self.concat_block('concat_4', x, 512, 3)
                x = self.concat_block('concat_5', x, 576, 3)
                x = self.concat_block('concat_6', x, 640, 3)
                x = self.concat_block('concat_7', x, 672, 3)
                x = self.concat_block('concat_8', x, 708, 3)
                x = self.concat_block('concat_9', x, 740, 3)
                x = self.concat_block('concat_10', x, 772, 3)
                x = self.concat_block('concat_11', x, 804, 3)
                x = self.concat_block('concat_12', x, 836, 3)
