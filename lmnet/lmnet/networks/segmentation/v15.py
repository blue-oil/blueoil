...
class LmSegnetV1(Base):
    ...
    def concat_block(self, name, inputs, channel, kernel):
        ...
        diff_channel = int(channel - in_ch/rate)
        with tf.variable_scope(name):
            x = self.lmnet_block('c_1', inputs, diff_channel * rate, kernel)
            x = self.lmnet_block('c_2', x, diff_channel, 1)

            x = tf.concat([inputs, x], axis=3)

            x = self.lmnet_block('c_3', x, channel, kernel)
    ...
    def context(self, x):
        with tf.variable_scope("context"):
            ...
            x = self.concat_block('concat_2', x, 64, 3)
            x = self.concat_block('concat_3', x, 64, 3)

            x = self._space_to_depth(name='s2d_2', inputs=x)
            x = self.lmnet_block('conv_4', x, 128, 1)
            x = self.concat_block('concat_5', x, 128, 3)
            x = self.concat_block('concat_6', x, 128, 3)
            # FIXME: need relu for convert.
            x = self.lmnet_block('conv_7', x, 128, 3, activation=tf.nn.relu)
            x_down_16 = x

            x = self.activation(x)
            x = self._space_to_depth(name='s2d_3', inputs=x)
            x = self.lmnet_block('conv_8', x, 256, 1)
            x = self.concat_block('concat_9', x, 256, 3)
            x = self.concat_block('concat_10', x, 512, 3)
            x = self.lmnet_block('conv_11', x, 512, 3, activation=tf.nn.relu)
            x_down_32 = x
