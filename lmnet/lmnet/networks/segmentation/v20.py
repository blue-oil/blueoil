...
class LmSegnetV1(Base):
    ...
    def spatial(self, x):
        with tf.variable_scope("spatial"):
            ...
            x = self.lmnet_block('conv_3', x, 256, 3, activation=tf.nn.relu)
    ...
    def context(self, x):
        with tf.variable_scope("context"):
            with tf.variable_scope("block_1"):
                ...
                x = self.lmnet_block('conv_4', x, 64, 3)

            with tf.variable_scope("block_2"):
                x = self._space_to_depth(name='s2d_2', inputs=x)
                x = self.lmnet_block('conv_5', x, 128, 1)
                x = self.lmnet_block('conv_6', x, 128, 3)
                x = self.lmnet_block('conv_7', x, 128, 3)
                x = self.lmnet_block('conv_8', x, 128, 3, activation=tf.nn.relu)
            ...
            with tf.variable_scope("block_3"):
                x = self._space_to_depth(name='s2d_3', inputs=x)
                x = self.lmnet_block('conv_9', x, 256, 1)
                x = self.lmnet_block('conv_10', x, 256, 3)
                x = self.lmnet_block('conv_11', x, 256, 3)
                x = self.lmnet_block('conv_12', x, 256, 3, activation=tf.nn.relu)
    ...
    def loss(self, output, labels):
        ...
        with tf.name_scope("loss"):
            ...
            tf.summary.scalar("auxiliary_loss_16", loss_cx_1)
            loss_cx_2 = self._cross_entropy(cx_2, labels, loss_weight) * self.auxiliary_weight
            tf.summary.scalar("auxiliary_loss_32", loss_cx_2)
