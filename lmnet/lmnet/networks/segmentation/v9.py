...
class LmSegnetV1(Base):
    ...
    def __init__(
        ...
        self.float_batch_norm_decay = 0.99
    ...
    def batch_norm(self, inputs, training, batch_norm_decay=None):
    ...
    def context(self, x):
        with tf.variable_scope("context"):
            ...
            x = self.lmnet_block('conv_10', x, 512, 3,
                                 activation=tf.nn.relu, batch_norm_decay=self.float_batch_norm_decay)
            x_down_32 = x
    ...
    def conv(self,
        ...
        with tf.variable_scope(name):
            ...
            x = self.batch_norm(conv, self.is_training, batch_norm_decay=batch_norm_decay)
            return x

    def fusion(self, sp, cx):
        with tf.variable_scope("fusion"):
            x = tf.concat([cx, sp], axis=3)
            x = self.lmnet_block('float_conv_base', x, self.num_classes, 1,
                                 activation=tf.nn.relu, batch_norm_decay=self.float_batch_norm_decay)
            stock = x
            h = x.get_shape()[1].value
            w = x.get_shape()[2].value
            ...
            x = self.lmnet_block('float_conv_1', x, self.num_classes, 1,
                                 activation=tf.nn.relu, batch_norm_decay=self.float_batch_norm_decay)
            x = self.lmnet_block('float_conv_2', x, self.num_classes, 1,
                                 activation=self.attention_act, batch_norm_decay=self.float_batch_norm_decay)
            x = stock * x
            x = stock + x
            return x
    ...
    def base(self, images, is_training, *args, **kwargs):
        ...
        x = lmnet_block('block_1', images, 32, 1, batch_norm_decay=self.float_batch_norm_decay)
        ...
        x = self.lmnet_block('float_last', x, self.num_classes, 1,
                             activation=None, batch_norm_decay=self.float_batch_norm_decay)

        # only for train
        self.cx_1 = self.conv("block_cx1", cx_1, self.num_classes, 1,
                              activation=None, batch_norm_decay=self.float_batch_norm_decay)
        self.cx_2 = self.conv("block_cx2", cx_2, self.num_classes, 1,
                              activation=None, batch_norm_decay=self.float_batch_norm_decay)
