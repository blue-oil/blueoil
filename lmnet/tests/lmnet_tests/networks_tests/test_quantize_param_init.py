...
def test_quantized_layers():
    ...
    for model in model_classes:
        ...
        with tf.variable_scope("scope1"):
            ...
            base, graph = quantizer.base(tf.ones([10, 32, 32, 3]), True)
            op_name_list = [op.name for op in graph.get_operations() if "kernel" in op.name and "scope1" in op.name]
        ...
        with tf.variable_scope("scope2"):
            ...
            base, graph = quantizer.base(tf.ones([10, 32, 32, 3]), True)
            op_name_list = [op.name for op in graph.get_operations() if "kernel" in op.name and "scope2" in op.name]
