        ...
        if self.op.op_type == 'QTZ_binary_mean_scaling':
            ...
            if op.is_quantized and nbit_qinput == 2:
                ...
                render_string = self.format_string(
                    ...
                    binConv2D_struct.layer_index = {self.qconv_idx};
                    ...
                    static T_UINT kernel_hwnocni{self.qconv_idx}[{k_size}] = {transpose_string};
                    vecCoefficient.emplace_back(kernel_hwnocni{self.qconv_idx});
                    {conv_func}({inputs_string}, {op.name}, scaling_factors::{op.name}, binConv2D_struct);
                    """
                )
                print(self.qconv_idx)
                self.qconv_idx += 1
