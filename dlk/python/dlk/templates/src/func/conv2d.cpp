...
void conv3x3_kn2row(const TensorView<T, MemoryLayout::NHWC>& input,
    const TensorView<T, MemoryLayout::HWOC>& kernels,
...
void ohwi_to_hwoi(const TensorView<T, MemoryLayout::OHWC>& ohwi,
    const TensorView<T, MemoryLayout::HWOC>& hwoi,
...
void conv1x1_kn2row(const TensorView<T, MemoryLayout::NHWC>& input,
                    const TensorView<T, MemoryLayout::OHWC>& kernels,
                    const TensorView<U, MemoryLayout::NHWC>& output,
                    struct convolution_parameters& p) {
  int ic = p.kernel_depth;
  int oc = p.output_channels;
  ...
  struct convolution_parameters p)
{
  ...
  if (p.kernel_height == 1 && p.kernel_width == 1 && p.padding == 0) {
    ...
    using hwoi_t = TensorView<T, MemoryLayout::HWOC>;
...
void func_Conv2D(const TensorView<T_FLOAT, MemoryLayout::NHWC>& input,
    const TensorView<T_FLOAT, MemoryLayout::OHWC>& weights,
