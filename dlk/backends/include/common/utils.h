...
unsigned pack_kernel_channel_wise(
  T_k k_data[],
  T_q k_data_packed[],
  int k_h,
  int k_w,
...
unsigned pack_input_channel_wise(
  T_in in_data[],
  T_q in_data_packed[],
  int in_h,
  int in_w,
...
unsigned pack_input_channel_wise_hp(
  T_in in_data[],
  T_q in_data_packed[],
  int in_h,
  int in_w,
  ...
  int pad)
{
  ...
  for (int ic = 0; ic < p::in_c; ic += p::nbits_per_word)
  {
    ...
    if (valid) { idx_in += p::nbits_per_word; }
  }
...
void kernel_transform_NHWC_to_NoCHWNi(
  T_q in_k[],
  T_q out_k[],
  unsigned kn,
  unsigned kh,
...
void kernel_transform_NHWC_to_NoHWCNi(
  T_q src[],
  T_q dst[],
  unsigned kn,
  unsigned kh,
...
void input_packed_transform_HWBC(
  T_q src[],
  T_q dst[],
  unsigned ih,
  unsigned iw,
...
template<class T>
void kernel_transform_NHWC_to_HWCN(
    T src[],
    T dst[],
    unsigned kn,
    unsigned kh,
