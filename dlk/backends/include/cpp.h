...
void qgemm(Tx A_packed[], Tx B_packed[], Ty Y[], u32 a_row, u32 a_col, u32 b_col);

void qconv_with_kn2row_impl(T_q in_data[], T_out out_data[], T_q k_data[], T_out partial_out_data[],
                            T_out threshold_data[], unsigned in_w, unsigned in_h, unsigned in_c, unsigned out_w,
                            unsigned out_h, unsigned out_c, unsigned k_w, unsigned k_h, unsigned pad);

template <int KH, int KW>
void qconv_with_kn2row(T_q in_data_packed[], T_out out_data[], T_q k_data_packed[], T_out threshold_data[],
                       ...
                       unsigned out_h, unsigned out_c, unsigned pad, unsigned stride)
{
  ...
  qconv_with_kn2row_impl(in_data_packed, out_data, k_data_packed, out_data, threshold_data, in_w, in_h, in_c_by_word,
...
template <int KH, int KW>
void a8w1_conv(T_in_k2c in_data[], T_out_k2c out_data[], T_k_k2c k_data[], unsigned in_w, unsigned in_h, unsigned in_c,
               unsigned out_w, unsigned out_h, unsigned out_c, unsigned pad, unsigned stride)
{
  ...
  if (KH == 3 && KW == 3) {
    a8w1_conv3x3_impl(in_data, out_data, k_data, in_w, in_h, in_c, out_w, out_h, out_c, pad, stride);
  }
}

template <int KH, int KW>
void a8w1_conv3x3_with_kn2row(T_in_k2c in_data[], T_out_k2c out_data[], T_k_k2c k_data[], unsigned in_w, unsigned in_h,
                              unsigned in_c, unsigned out_w, unsigned out_h, unsigned out_c, unsigned pad,
                              unsigned stride)
{
  assert((KH == 3) && (KW == 3));
  assert(((KH == 3) && (pad == 1)) || ((KH == 1) && (pad == 0)));
  ...
  if (KH == 3 && KW == 3) {
    a8w1_conv3x3_with_kn2row_impl(in_data, out_data, k_data, out_data, in_w, in_h, in_c, out_w, out_h, out_c, pad);
  }
}
} // namespace cpp
