        ...
        for (int ih_low = 0; ih_low < p::in_tile_h; ++ih_low)
        {
          for (int iw_low = 0; iw_low < p::in_tile_w; ++iw_low)
          {
            ...
            for (int ic = 0; ic < in_c_by_word; ic++)
            {
              ...
              {
                int idx_in = ih * in_w * in_c_by_word * in_b + iw * in_c_by_word
  * in_b + ic * in_b f + ib; in_buf[ih_low][iw_low][ic][ib] = (input_on) ?
  in_data[idx_in] : T_q(0);
                // printf("in_buf[%d][%d][%d] = %d\n", ih_low, iw_low, ic,
                //        int(in_buf[ih_low][iw_low][ic]));
              }
            }
  ...
  parts. int idx_k = (kh * k_w * in_c_by_word * out_c_low) + (kw * in_c_by_word
  * out_c_low) + (ic * out_c_low) + oc + (oc_high * k_h * k_w * in_c_by_word);
                  k_buf[ic][oc] = k_data[idx_k];
                }
              }
            }
