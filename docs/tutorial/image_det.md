...
Train your model by running `blueoil train` with a model configuration.
	...
	$ docker run --rm -e CUDA_VISIBLE_DEVICES=0 -v $(pwd)/openimages_face:/home/blueoil/openimages_face -v $(pwd)/config:/home/blueoil/config -v $(pwd)/saved:/home/blueoil/saved blueoil_$(id -un):{TAG} blueoil train -c config/my_config.yml
